...
<template>
  <div class="container" :class="cc()">
    <banner-box pic="cbg6" :pid="pid" :title="$t('job.title')" :sub="$t('job.sub')"/>
    <div class="main bg3">
      <div class="max-width bs round22 sysBColor-fff">
         <div class="mt-font-16" :class="{'mt-font-22':!isMobile}">
           <div class="dfj">
         ...
         <div class="work-list">
            <div class="work-list-text dfj mt-font-14" :class="{'mt-font-22':!isMobile}">
    ...
    <!-- <div class="mst-margin-t-30 mst-margin-b-30">
      <Btn :name="$t('btns.b')[4]" w="260px" h="58px"/>
    </div> -->
     <div class="max-width">
      <Title :title="$t('job.increase.title')" />
      <p v-html="$t('job.increase.sub')"></p>
      <Hold />
      <a-row type="flex" class="max-width1000">
        <a-col v-for="(item, index) in $t('job.increase.list')" :key="index" :xs="{ span: 12 }" :sm="{ span: 6}" >
          <a-progress type="circle" :percent="percent[index]" strokeColor="#001e4f" class="mt-font-35 mt-font-w sysgreen"/>
          <p class="mst-margin-t-20 mt-font-22">{{item}}</p>
        </a-col>
      </a-row>
      <Hold />
      <Hold />
    ...
    <div class="bg4">
      <div class="max-width">
        <Hold from="s30"/>
      <Title :title="$t('advantage.title')" />
      <Gird :data="$t('job.advantage.list')" :gird="gird" from="job" :start="start"/>
      <self-test :left="$t('work.list1')[4]" :right="$t('work.list1')[6]" from="work" :btn="$t('work.list1')[5]"/>
      </div>
    </div>
        ...
        <a-col :span="15" class="br">
          <div v-for="(item, index) in  $t('work.list')" :key="index" class="bb clearfix">
            ...
            <a-col :span="15" class="com dfj mst-color-999">
              ...
              <div v-if="index == 3" class="mst-padded-20 textarea scroll">{{datas.descripe}}</div>
...
export default {
  ...
  data () {
    return {
      ...
      gird: 6,
       ...
       datas: {},
      jobSub: '从硬技能到软实力，明途求职皆在帮助每一位学员全面提升求职竞争力，与学员分享求职经验与技巧，<br/>在HR面前脱颖而出，新获心仪offer。'
    }
  },
  computed: {
    tabs() {
  ...
  methods: {
    async fetchList() {
      let res = await api.work.index({type: 'plan'});
      if(res) {
        console.log(res);
